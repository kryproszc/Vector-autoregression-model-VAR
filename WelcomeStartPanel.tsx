	...
	const recommendationOverviewData = useMemo<StageOverviewSlice[]>(
		() =>
			orderedRecommendationGroups.map((group) => ({
				stageGroupCode: group.stageGroupCode,
				stageGroupLabel: group.stageGroupShortLabel,
				count: group.count,
			})),
											...
											>
												{column.key === "zakresInspekcji" ? (
													...
													(() => {
														...
														const displayedStatus = shortenDuplicatedStatusLabel(
															String(row[column.key] ?? "-"),
														);

														return (
															<div className="flex items-start justify-between gap-2">
																<span className="whitespace-normal break-words">{displayedStatus}</span>
																{showIcon ? (
											...
											{RECOMMENDATION_TABLE_COLUMNS.map((column) => (
												...
												>
													{column.key === "terminWykonaniaZalecen" ? (
														...
														column.key === "status"
															? String(row.statusSkrot ?? row.status ?? "-")
															: String(row[column.key] ?? "-")
													)}
												</td>
											))}
