...
export function TimeReportPanel({
	...
	const chartPointsData = useMemo(() => {
		const pointsByYear = new Map<number, typeof scatterRows>();

		for (const point of scatterRows) {
	...
	const yAxisDomain = useMemo<[number, number]>(() => {
		...
		const values = scatterRows.map((point) => point.time);
	...
	const yearRangeAreas = useMemo(() => {
		return chartTrendData.map((yearData) => {
	...
	const histogramAreas = useMemo(() => {
		if (yearRangeAreas.length === 0 || scatterRows.length === 0) {
			return [] as Array<{
				key: string;
				x1: number;
				x2: number;
	...
	const sortedDetailRows = useMemo(() => detailRows, [detailRows]);
	...
	return (
		<section className="flex h-[calc(100vh-4.4rem)] min-h-0 flex-col space-y-4 overflow-hidden rounded-2xl border border-slate-700/70 bg-[#101f39] p-5 shadow-[0_18px_44px_rgba(2,8,23,0.34)]">
			...
			<div className="space-y-2">
				...
				<div
					className={`subtle-horizontal-scroll subtle-vertical-scroll w-full max-w-full overflow-x-auto rounded-xl border border-slate-300 bg-white shadow-[0_10px_28px_rgba(2,8,23,0.18)] ${
						...
						{scatterRows.length > 0 ? (
							...
							>
								...
								<ResponsiveContainer width="100%" height="100%">
									<ComposedChart data={chartTrendData} margin={{ top: 8, right: 12, left: 8, bottom: 28 }}>
										...
										<Tooltip
											content={({ active, payload }) => {
												...
												const pointPayload = payload.find(
													(item) => item?.dataKey === "time",
												)?.payload as
													| {
															year: number;
															time: number;
												...
												if (pointPayload) {
													return (
														<div className="rounded-md border border-[#b6c6dc] bg-white px-3 py-2 text-[#1a3559] text-xs shadow-[0_8px_18px_rgba(2,8,23,0.18)]">
															<p><strong>Nazwa podmiotu:</strong> {pointPayload.nazwaPodmiotu}</p>
															<p><strong>Kontrola:</strong> {pointPayload.kontrola}</p>
															<p><strong>Osoba kierująca:</strong> {pointPayload.osobaKierujaca}</p>
															<p><strong>Zespół:</strong> {pointPayload.zespol}</p>
															<p><strong>Czas:</strong> {pointPayload.time}</p>
														</div>
													);
												}

												const linePayload = payload.find(
													(item) => item?.dataKey === "trend",
												)?.payload as { year: number; trend: number } | undefined;
												...
												return (
													<div className="rounded-md border border-[#b6c6dc] bg-white px-3 py-2 text-[#1a3559] text-xs shadow-[0_8px_18px_rgba(2,8,23,0.18)]">
														<p><strong>Rok:</strong> {linePayload.year}</p>
														<p>
															<strong>{trendMode === "average" ? "Średnia" : "Mediana"}:</strong>{" "}
															{linePayload.trend.toFixed(1)}
														</p>
													</div>
												);
											}}
										...
										<Line
											...
											stroke="#255087"
											strokeWidth={3}
											dot={<Dot r={3} fill="#255087" />}
											isAnimationActive={false}
										/>
										{showScatterPoints ? (
											<Scatter data={chartPointsData} dataKey="time" fill="#5d8fc9" />
										) : null}
									</ComposedChart>
								</ResponsiveContainer>
								</div>
			...
			<div className="space-y-2">
				...
				<div className="relative subtle-horizontal-scroll subtle-vertical-scroll w-full max-w-full overflow-x-auto overflow-y-auto rounded-xl border border-slate-300 bg-white shadow-[0_10px_28px_rgba(2,8,23,0.18)]">
					{isLoading ? <TableLoadingOverlay /> : null}
					<table className="w-full min-w-max border-collapse text-slate-900 text-sm">
						<thead>
							<tr className="bg-slate-100 text-slate-800">
