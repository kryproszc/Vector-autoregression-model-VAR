...
class InspectionsDetailedRow(BaseModel):
	...
	stage_group_label: str
	stage_group_order: int
	stage_subgroup_code: str
	stage_subgroup_label: str
	stage_subgroup_order: int
...
STAGE_GROUPS: list[dict[str, Any]] = [
	...
	},
	{
		...
		"subgroups": [
			...
			{"code": "in_progress_report_writing", "label": "Piszemy", "order": 2},
		],
	},
	{
		"code": "post",
	...
	},
	{
		...
		"order": 4,
		"subgroups": [
			{"code": "rec_writing_recommendations", "label": "Piszemy zalecenia", "order": 1},
...
def get_inspections_time_analytics(
	...
	for raw_row in rows:
		...
		if row_type not in {"K", "W"}:
			continue
		if _is_hidden_status_code(row.get("status_inspekcji_kod")):
			continue
...
def get_inspections_matrix(
	...
	for row in rows:
		if not _can_access_inspection_for_reports(conn, dict(row), operator):
			continue
...
def get_inspections_detailed(
	...
	for row in rows:
		...
		stage_payload = _stage_payload_from_status_code(row["status_inspekcji_kod"])
		...
		is_leader_in_manager_team = _is_leader_in_manager_scope(conn, leader_user_id, operator)
		is_member_in_manager_team = _is_member_in_manager_scope(conn, inspection_id, operator)
		...
		result_rows.append(
			{
				...
				"stage_group_code": stage_payload["stage_group_code"],
				"stage_group_label": stage_payload["stage_group_label"],
				"stage_group_order": stage_payload["stage_group_order"],
				"stage_subgroup_code": stage_payload["stage_subgroup_code"],
				"stage_subgroup_label": stage_payload["stage_subgroup_label"],
				"stage_subgroup_order": stage_payload["stage_subgroup_order"],
...
def get_inspections_stage_summary(
	...
	with get_connection() as conn:
		...
		for group in STAGE_GROUPS:
			...
			group_stats[group_code] = {
				"stageGroupCode": group_code,
				"stageGroupLabel": str(group["label"]),
				"stageGroupOrder": int(group["order"]),
				"count": 0,
				"countTeam": 0,
				"countManagerAdded": 0,
				...
				"subgroups": {
					sub["code"]: {
						"stageSubgroupCode": str(sub["code"]),
						"stageSubgroupLabel": str(sub["label"]),
						"stageSubgroupOrder": int(sub["order"]),
						"count": 0,
						"countTeam": 0,
						"countManagerAdded": 0,
						"countTeamAndManagerAdded": 0,
		...
		for row in rows:
			...
			stage_payload = _stage_payload_from_status_code(row_dict.get("status_inspekcji_kod"))
			group_code = str(stage_payload["stage_group_code"])
			subgroup_code = str(stage_payload["stage_subgroup_code"])

			if subgroup_code == "unknown_unmapped":
				quality_error_count += 1

			total_inspections += 1
			group_item = group_stats[group_code]
			subgroup_item = group_item["subgroups"][subgroup_code]

			group_item["count"] += 1
			subgroup_item["count"] += 1

			if manager_context is not None:
				if in_team_scope:
					group_item["countTeam"] += 1
					subgroup_item["countTeam"] += 1
				if in_added_scope:
					group_item["countManagerAdded"] += 1
					subgroup_item["countManagerAdded"] += 1
				if in_team_scope and in_added_scope:
					group_item["countTeamAndManagerAdded"] += 1
					subgroup_item["countTeamAndManagerAdded"] += 1

		groups_payload: list[dict[str, Any]] = []
		for group in sorted(group_stats.values(), key=lambda item: int(item["stageGroupOrder"])):
			subgroups_payload = sorted(
				group["subgroups"].values(),
				key=lambda item: int(item["stageSubgroupOrder"]),
			)
			groups_payload.append(
				{
					"stageGroupCode": group["stageGroupCode"],
					"stageGroupLabel": group["stageGroupLabel"],
					"stageGroupOrder": group["stageGroupOrder"],
					"count": group["count"],
					"countTeam": group["countTeam"],
					"countManagerAdded": group["countManagerAdded"],
					"countTeamAndManagerAdded": group["countTeamAndManagerAdded"],
	...
	return {
		...
		"stageDictionaryVersion": STAGE_DICTIONARY_VERSION,
	...
	with get_connection() as conn:
		...
		status_rows = conn.execute(
			...
			SELECT
				...
				NULLIF(trim(skrot_pozycji), '') AS skrot_pozycji,
		...
		for status_row in status_rows:
			...
			group_stats[status_code] = {
				...
				"stageGroupShortLabel": status_row["skrot_pozycji"] or status_row["nazwa_pozycji"],
	...
	with get_connection() as conn:
		...
		rows = conn.execute(
			...
			SELECT
				...
				COALESCE(np_rec.nazwa_pozycji, np_ins.nazwa_pozycji, 'brak') AS nazwa_podmiotu,
				i.created_by_user_id AS inspection_created_by_user_id,
				ru.zespol_id AS recommendation_author_team_id,
				ru.created_by_user_id AS recommendation_author_created_by_user_id,
				(
				...
				COUNT(*) OVER (PARTITION BY r.inspection_id) AS liczba_zalecen
			FROM recommendations r
			LEFT JOIN inspections i ON i.id = r.inspection_id
			LEFT JOIN users ru ON ru.id = r.created_by_user_id
